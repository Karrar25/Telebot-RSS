...
async def fetch_and_post():
    ...
    # إرسال رسالة تأكيد عند التشغيل لأول مرة (للتأكد من الربط)
    if not sent_news:
        try:
            await bot.send_message(chat_id=CHANNEL_ID, text="✅ تم تشغيل البوت بنجاح وهو الآن يراقب الأخبار العاجلة...")
            ...
            for entry in feed.entries[:5]:
                ...
                if news_id not in sent_news:
                    ...
                    if is_important(title, summary):
                        ...
                        # حفظ الخبر كأنه أرسل لكي لا يفحصه مرة أخرى
                        sent_news.append(news_id)
                        save_sent_news(sent_news)
        except Exception as e:
